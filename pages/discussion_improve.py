...
import zoneinfo
from common import display_header, get_system_prompt, go_to, push_to_github
...
def add_message2(role, content, extra=None):
    ...
    if role == "user":
        start = st.session_state.get("chat_timer_start2")
        if start:
        ...
        message["index"] = st.session_state.user_utter_index2
        st.session_state.user_utter_index2 += 1
    else:
        st.session_state.chat_timer_start2 = datetime.now(jst)
        message["delay"] = ""
...
- **ディスカッションを終了するときは必ず本ページ右下の「ディスカッションを終了する」ボタンを押してください。**
""")
...
if not st.session_state.messages2:
    st.session_state.messages2 = [
        {"role": "system",
    ...
    if st.session_state.chat_timer_start2 is None:  #初回のみ
        st.session_state.chat_timer_start2 = datetime.now(jst)
else:
    if st.session_state.messages2[0]["role"] == "system":
        st.session_state.messages2[0]["content"] = get_system_prompt(
...
with input_col:
    prompt = st.text_input(
        "テキストを入力してください",
        value="",
        key=f"chat_input2_{st.session_state.input_counter2}",
...
if send_button and prompt:
    ...
    add_message2("assistant", reply)

    st.session_state.input_counter2 += 1
    st.rerun()

def go_survey2():
    # 1. チャット時間を計算
    start = st.session_state.chat_start_time2
    if start:
        elapsed = datetime.now() - start
        minutes, seconds = divmod(int(elapsed.total_seconds()), 60)
        st.session_state.chat_duration2 = f"{minutes}分{seconds}秒"
    ...
    log_text += f"\n⏱ チャット滞在時間: {st.session_state.chat_duration2}"

    # 3. GitHub に送信（ファイル名に_2を追加）
    jst = zoneinfo.ZoneInfo("Asia/Tokyo")
    now = datetime.now(jst)
    filename = f"log/{st.session_state.username}_2_{now.strftime('%Y%m%d_%H%M%S')}.txt"
    ...
    st.switch_page("pages/survey_improve.py")
